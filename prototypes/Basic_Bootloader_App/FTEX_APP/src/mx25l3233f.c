/**
  * @file    mx25l3233f.c
  * @brief   This file contain the firmware driver for the 
						 external flash MX25.
  */
 ...
 **/
int32_t MX25L3233F_ChipErase(void)
{
  /*!< Select the FLASH: Chip Select low */
  FLASH_SPI_CS_LOW();
  
  /*!< Send "Write Enable" instruction */
  FLASH_SPI_IO_WriteByte(MX25L3233F_CHIP_ERASE_CMD);
...
/**
  ...
  */
int32_t MX25L3233F_PageProgram(uint8_t *pData, uint32_t Address, uint32_t Size)
{
  if(Size == 0) return MX25L3233F_ERROR_PARAMETER;
  ...
  /*!< Send "Write Enable" instruction */
  FLASH_SPI_IO_WriteByte(MX25L3233F_PAGE_PROG_CMD);
  ...
  for(uint32_t i = 0; i < Size; i++) FLASH_SPI_IO_WriteByte(*(pData++));
 ...
 */
int32_t MX25L3233F_BlockErase(uint32_t BlockAddress, MX25L3233F_EraseTypeDef BlockSize)
{
  /*!< Select the FLASH: Chip Select low */
  FLASH_SPI_CS_LOW();
  ...
  case MX25L3233F_ERASE_4K :
    FLASH_SPI_IO_WriteByte(MX25L3233F_SECTOR_ERASE_4K_CMD);
    break;

  case MX25L3233F_ERASE_32K :
    FLASH_SPI_IO_WriteByte(MX25L3233F_BLOCK_ERASE_32K_CMD);
    break;

  case MX25L3233F_ERASE_64K :
    FLASH_SPI_IO_WriteByte(MX25L3233F_BLOCK_ERASE_64K_CMD);
    break;

  case MX25L3233F_ERASE_CHIP :
    return MX25L3233F_ChipErase();
  ...
  /*!< Send SectorAddr high address byte */
  FLASH_SPI_IO_WriteByte((BlockAddress >> 16) & 0xFF);
  /*!< Send SectorAddr medium address byte */
  FLASH_SPI_IO_WriteByte((BlockAddress >> 8) & 0xFF);
  /*!< Send SectorAddr low address byte */
  FLASH_SPI_IO_WriteByte(BlockAddress & 0xFF);
...
**/
int32_t MX25L3233F_WriteEnable(void)
{
  /*!< Select the FLASH: Chip Select low */
  FLASH_SPI_CS_LOW();
  
  /*!< Send "Write Enable" instruction */
  FLASH_SPI_IO_WriteByte(MX25L3233F_WRITE_ENABLE_CMD);
