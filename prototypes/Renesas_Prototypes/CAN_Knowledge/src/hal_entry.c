...
void hal_entry(void)
{
    ...
    if(FSP_SUCCESS != err)
    {
		...
		err = R_CAN_ModeTransition(&g_can0_ctrl,CAN_OPERATION_MODE_NORMAL,CAN_TEST_MODE_LOOPBACK_EXTERNAL);
		if(FSP_SUCCESS != err)
    {
			while(1);
    }
		...
		err = R_CAN_CallbackSet(&g_can0_ctrl,can_callback,NULL,NULL);
		if(FSP_SUCCESS != err)
    {
			while(1);
    }
