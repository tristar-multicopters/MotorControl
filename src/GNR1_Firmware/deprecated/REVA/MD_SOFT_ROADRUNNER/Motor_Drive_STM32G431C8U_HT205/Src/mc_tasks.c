...
OpenLoop_Handle_t * pOpenLoopM1 = &OpenLoopM1;
OLVars_t OLVars[NBR_OF_MOTORS];

MCI_Handle_t savedMci[NBR_OF_MOTORS];
MCT_Handle_t savedMCT[NBR_OF_MOTORS];
...
//	PIDIqHandle_M1.hDefKpGain = (int16_t)Flash_ReadConfig(CONFIG_REG_TORQUE_KP).quarter[0];
//	PIDIqHandle_M1.hDefKiGain = (int16_t)Flash_ReadConfig(CONFIG_REG_TORQUE_KI).quarter[0];
//	PIDIdHandle_M1.hDefKpGain = (int16_t)Flash_ReadConfig(CONFIG_REG_FLUX_KP).quarter[0];
//	PIDIdHandle_M1.hDefKiGain = (int16_t)Flash_ReadConfig(CONFIG_REG_FLUX_KI).quarter[0];
//	
//	TempSensorParamsM1.hOverTempThreshold = (uint16_t)Flash_ReadConfig(CONFIG_REG_TEMPERATURE_MAX).quarter[0];
//	RealBusVoltageSensorParamsM1.OverVoltageThreshold = (uint16_t)Flash_ReadConfig(CONFIG_REG_DCBUS_MAX).quarter[0];
//	RealBusVoltageSensorParamsM1.UnderVoltageThreshold = (uint16_t)Flash_ReadConfig(CONFIG_REG_DCBUS_MIN).quarter[0];
//	PWM_Handle_M1.pParams_str->DAC_OCP_Threshold = (uint16_t)Flash_ReadConfig(CONFIG_REG_IPH_MAX).quarter[0];
//	
//	HALL_M1.PhaseShift = (uint16_t)Flash_ReadConfig(CONFIG_REG_HALL_PHASESHIFT).quarter[0];
//	HALL_M1._Super.bElToMecRatio = (uint16_t)Flash_ReadConfig(CONFIG_REG_POLEPAIRS).quarter[0];

//	FF_M1.wDefConstant_2 = (int32_t)Flash_ReadConfig(CONFIG_REG_KFF).half[0];
  ...
  FOC_Clear(M1);
  FOCVars[M1].bDriveInput = INTERNAL;//EXTERNAL;
  ...
  /* USER CODE BEGIN MCboot 2 */
	...
	OL_Init( pOpenLoopM1, MCT[M1].pSpeedSensorMain );
...
uint16_t OL_ControllerM1(void)
{
  ...
  speedHandle = STC_GetSpeedSensor(pSTC[M1]);
  hElMesAngle = SPD_GetElAngle(speedHandle);
  PWMC_GetPhaseCurrents(pwmcHandle[M1], &Iab);
  RCM_ReadOngoingConv();
  RCM_ExecNextConv();
  Ialphabeta = MCM_Clarke(Iab);
  Iqd = MCM_Park(Ialphabeta, hElMesAngle);
	
	OL_Calc(pOpenLoopM1);
  Vqd = OL_VqdConditioning(pOpenLoopM1);
  Vqd = Circle_Limitation(pCLM[M1], Vqd);
  Valphabeta = MCM_Rev_Park(Vqd, pOpenLoopM1->hElAngle);
  hCodeError = PWMC_SetPhaseVoltage(pwmcHandle[M1], Valphabeta);
  OLVars[M1].Vqd = Vqd;
  OLVars[M1].Iab = Iab;
  OLVars[M1].Ialphabeta = Ialphabeta;
  OLVars[M1].Iqd = Iqd;
  OLVars[M1].Valphabeta = Valphabeta;
  OLVars[M1].hElMesAngle = hElMesAngle;
	OLVars[M1].hElCmdAngle = pOpenLoopM1->hElAngle;
  return(hCodeError);
}
