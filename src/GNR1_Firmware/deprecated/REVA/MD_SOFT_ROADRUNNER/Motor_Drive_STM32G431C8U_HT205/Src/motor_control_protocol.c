  ...
  uint8_t bErrorCode;
	...
	if (pHandle->ConfigMode == false)
	{
		...
		case MC_PROTOCOL_CODE_GET_MP_INFO:
			...
			case MC_PROTOCOL_CODE_ENTER_CONFIG:
			{
				for (uint8_t i = 0; i<CONFIG_REG_NUM; i++) 
				{
					pHandle->config_localreglist.received[i] = false;
					pHandle->config_localreglist.value[i].whole = 0;
				}
				
				pHandle->ConfigMode = true;
			}
			break;
	...
	else
	{
		switch (Code)
		{
			case MC_PROTOCOL_CODE_EXIT_CONFIG:
			{
				pHandle->ConfigMode = false;
			}
			break;
			
			case MC_PROTOCOL_CODE_CONFIG_OVERWRITE:
			{
				Flash_EraseAllConfig();
				for (uint8_t i = 0; i<CONFIG_REG_NUM; i++) 
				{
					Flash_WriteConfig(pHandle->config_localreglist.value[i], (Config_Reg_Name_t)i);
				}
			}
			break;
			
			case MC_PROTOCOL_CODE_SET_CONFIG_REG:
			{
				Config_Reg_Name_t bRegID = (Config_Reg_Name_t)buffer[0];
				
				switch (bRegID)
				{
					case CONFIG_REG_TORQUE_KP:
					case CONFIG_REG_TORQUE_KI:
					case CONFIG_REG_FLUX_KP:
					case CONFIG_REG_FLUX_KI:
					case CONFIG_REG_TEMPERATURE_MAX:
					case CONFIG_REG_DCBUS_MAX:
					case CONFIG_REG_DCBUS_MIN:
					case CONFIG_REG_IPH_MAX:
					case CONFIG_REG_HALL_PHASESHIFT:
					case CONFIG_REG_POLEPAIRS:
					{
						pHandle->config_localreglist.received[bRegID] = true;
						pHandle->config_localreglist.value[bRegID].quarter[0] = buffer[1] + (buffer[2]<<8);
					}
					case CONFIG_REG_KFF:
					{
						pHandle->config_localreglist.received[bRegID] = true;
						pHandle->config_localreglist.value[bRegID].half[0] = buffer[1] + (buffer[2]<<8) + (buffer[3]<<16) + (buffer[2]<<24);
					}
					default:
					{}
				}
			...
			case MC_PROTOCOL_CODE_GET_CONFIG_REG:
			{
				Config_Reg_Name_t bRegID = (Config_Reg_Name_t)buffer[0];
				...
				switch (bRegID)
				{
					...
					case CONFIG_REG_POLEPAIRS:
					{
						Config_Reg_Value_t value = Flash_ReadConfig(bRegID);
						pHandle->fFcpSend(pHandle->pFCP, ACK_NOERROR, (uint8_t*)(&value.quarter[0]), 2);
						RequireAck = false;
					}
					case CONFIG_REG_KFF:
					{
						Config_Reg_Value_t value = Flash_ReadConfig(bRegID);
						pHandle->fFcpSend(pHandle->pFCP, ACK_NOERROR, (uint8_t*)(&value.quarter[0]), 4);
						RequireAck = false;
					}
					default:
					{}
