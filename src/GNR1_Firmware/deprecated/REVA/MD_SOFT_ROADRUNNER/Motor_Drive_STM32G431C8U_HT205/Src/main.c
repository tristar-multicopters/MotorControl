...
uint8_t UART3_rxBuffer[1] = {0};

int16_t loggingTemperature[LOGGING_SIZE];
int32_t loggingSpeed[LOGGING_SIZE];
...
static void MX_TIM3_Init(void)
{
  ...
  HAL_TIMEx_RemapConfig(&htim3, TIM_TIM3_ETR_COMP2);
  sConfigOC.OCMode = TIM_OCMODE_ACTIVE;
  sConfigOC.Pulse = 0x0005;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
...
static void MX_TIM4_Init(void)
{
  ...
  HAL_TIMEx_RemapConfig(&htim4, TIM_TIM4_ETR_COMP4);
  sConfigOC.OCMode = TIM_OCMODE_ACTIVE;
  sConfigOC.Pulse = 0x0005;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
...
void startLoggingTask(void *argument)
{
	MCI_Handle_t * hMCI = GetMCI(M1);
	MCT_Handle_t * hMCT = GetMCT(M1);
  ...
  {	
		if (i == 0)
		{
			...
			{
				loggingTemperature[i] = NTC_GetAvTemp_C(hMCT->pTemperatureSensor);
				loggingSpeed[i] = SPD_GetElSpeedDpp(hMCT->pSpeedSensorMain);
				osDelay(10000);
			}
		}
    osDelay(100);
...
void startThrottleTask(void *argument)
{
  /* USER CODE BEGIN startThrottleTask */
	...
	Derate_Handle_t * pDerateM1 = &DerateM1;
	...
	Derate_Init(pDerateM1);
	
//	MCI_ExecTorqueRamp( hMCI, 0, 0 );
//	MCI_StartMotor( hMCI );
...
//		torqueref = Throttle_CalcIqref(pThrottleSensor);
//		Derate_CalcTorque(pDerateM1, torqueref);
//		MCI_ExecTorqueRamp( hMCI, torqueref, 0 );
		
//		elOmega ++;
//		OL_UpdateElSpeed( pOpenLoopM1, elOmega );
