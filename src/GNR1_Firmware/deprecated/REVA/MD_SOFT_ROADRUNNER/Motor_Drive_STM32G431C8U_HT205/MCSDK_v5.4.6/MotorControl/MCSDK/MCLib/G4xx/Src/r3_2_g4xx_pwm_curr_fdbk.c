...
__weak void R3_2_Init( PWMC_R3_2_Handle_t * pHandle )
{
  ...
  ADC_TypeDef * ADCx_2 = pHandle->pParams_str->ADCx_2;
	TIM_TypeDef * TIM_OCPAx = pHandle->pParams_str->TIM_OCPa;
	TIM_TypeDef * TIM_OCPBx = pHandle->pParams_str->TIM_OCPb;
	TIM_TypeDef * TIM_OCPCx = pHandle->pParams_str->TIM_OCPc;
  ...
  if ( ( uint32_t )pHandle == ( uint32_t )&pHandle->_Super )
  {
    ...
    if ( COMP_OCPAx != NULL )
    {
      ...
      LL_COMP_Lock( COMP_OCPAx );
			...
			/* Delay timer setup */
			LL_TIM_EnableCounter(TIM_OCPAx);
			LL_TIM_OC_SetCompareCH1(TIM_OCPAx, pHandle->pParams_str->Tdelay_OCP);
			LL_TIM_EnableIT_CC1(TIM_OCPAx);
    }
    ...
    if ( COMP_OCPBx != NULL )
    {
      ...
      LL_COMP_Lock( COMP_OCPBx );
			...
			/* Delay timer setup */
			LL_TIM_EnableCounter(TIM_OCPBx);
			LL_TIM_OC_SetCompareCH1(TIM_OCPBx, pHandle->pParams_str->Tdelay_OCP);
			LL_TIM_EnableIT_CC1(TIM_OCPBx);
    }
    ...
    if ( COMP_OCPCx != NULL )
    {
      ...
      LL_COMP_Lock( COMP_OCPCx );
			...
			/* Delay timer setup */
			LL_TIM_EnableCounter(TIM_OCPCx);
			LL_TIM_OC_SetCompareCH1(TIM_OCPCx, pHandle->pParams_str->Tdelay_OCP);
			LL_TIM_EnableIT_CC1(TIM_OCPCx);
    }
