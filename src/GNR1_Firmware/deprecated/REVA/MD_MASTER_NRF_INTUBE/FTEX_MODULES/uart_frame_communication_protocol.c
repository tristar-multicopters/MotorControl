...
static nrf_drv_uart_t uart_ufcp_inst = NRF_DRV_UART_INSTANCE(UART_UFCP_INSTANCE);
...
void UFCP_Init( nrf_drv_uart_config_t uart_config, ufcp_event_handler_t evt_handler)
{
  ...
  FCP_Init( & ufcp_handle._Super );
	...
	nrf_drv_uart_init(&uart_ufcp_inst, & ufcp_handle.uart_config, uart_event_handler);	
}
    ...
    switch ( pBaseHandle->RxFrameLevel )
    {
      case 0: // First Byte received --> The Code
        ...
        pBaseHandle->RxFrameLevel++;
				...
				nrf_drv_uart_rx(&uart_ufcp_inst, rx_buffer, 1);
        break;
      ...
      default: // In the payload or the "CRC"
        ...
        else
        {
          ...
          nrf_drv_uart_rx_disable(&uart_ufcp_inst);//LL_USART_DisableIT_RXNE(pHandle->USARTx); *********************************
...
void UFCP_TX_IRQ_Handler(void)
{
  ...
  if ( FCP_TRANSFER_IDLE != pBaseHandle->TxFrameState )
  {
    ...
    nrf_drv_uart_tx(&uart_ufcp_inst, &tx_data8, 1); //LL_USART_TransmitData8(pHandle->USARTx, tx_data); *********************************
...
uint8_t UFCP_Receive(void)
{
  ...
  if ( FCP_TRANSFER_IDLE == pBaseHandle->RxFrameState )
  {
    ...
    pBaseHandle->RxFrameState = FCP_TRANSFER_ONGOING;
		...
		nrf_drv_uart_rx_enable(&uart_ufcp_inst);
    nrf_drv_uart_rx(&uart_ufcp_inst, rx_buffer, 1); //LL_USART_EnableIT_RXNE(pActualHandle->USARTx); ***************************
...
uint8_t UFCP_Send(uint8_t code, uint8_t *buffer, uint8_t size)
{
  ...
  if ( FCP_TRANSFER_IDLE == pBaseHandle->TxFrameState )
  {
    ...
    pBaseHandle->TxFrameState = FCP_TRANSFER_ONGOING;
		...
		nrf_drv_uart_tx(&uart_ufcp_inst, & pBaseHandle->TxFrame.Code, 1);
