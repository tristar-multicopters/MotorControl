...
__weak void HALL_Init( HALL_Handle_t * pHandle )
{
  ...
  */
  ...
  for ( bIndex = 0u; bIndex < bSpeedBufferSize; bIndex++ )
  {
	...
	/* Erase Debugging buffers */
	
	  for ( bIndex = 0u; bIndex < 9; bIndex++ )
  {
    pHandle->_Super.buffer_1[bIndex]  = 0;
  }

//	pHandle->Max_CaptureBuffer_Delta = 92798976 / 2000 ;
...
__weak void HALL_Clear( HALL_Handle_t * pHandle )
{
  ...
  HALL_Init_Electrical_Angle( pHandle );
	...
	pHandle->_Super.buffer_pointer = 0;				// User edit
	
	pHandle->error_1 = false;
	pHandle->error_2 = false;
	pHandle->error_3 = false;
	pHandle->error_4 = false;
	pHandle->error_5 = false;
	
	pHandle->_Super.error_1 = false;
	pHandle->_Super.error_2 = false;
	pHandle->_Super.error_3 = false;
	pHandle->_Super.error_4 = false;
	
	pHandle->AvrElSpeedDpp_medium = 0;
	pHandle->AvrMechSpeedUnit_medium = 0;
	
	pHandle->wCaptBuf_Prev = 0;
      ...
      else
      {
        ...
        else
        {
          *hMecSpeedUnit = ( int16_t )pHandle->SatSpeed;
					pHandle->error_3 = true;
        }
      }
    }
					 pHandle->AvrMechSpeedUnit_medium = *hMecSpeedUnit;
		       pHandle->AvrElSpeedDpp_medium = pHandle->AvrElSpeedDpp;
		
    bReliability = SPD_IsMecSpeedReliable( &pHandle->_Super, hMecSpeedUnit );
		if(!bReliability)
		{		
		pHandle->error_2 = true;}
  }
  else
  {
    bReliability = false;
    pHandle->_Super.bSpeedErrorNumber = pHandle->_Super.bMaximumSpeedErrorsNumber;
		pHandle->error_1 = true;
...
__weak void * HALL_TIMx_CC_IRQHandler( void * pHandleVoid )
{
  ...
  if ( pHandle->SensorIsReliable )
  {
    ...
    bPrevHallState = pHandle->HallState;
		pHandle->_Super.previous_previous_state = pHandle->_Super.previous_state;
		pHandle->_Super.previous_state = pHandle->HallState;
    ...
    switch ( pHandle->HallState )
    {
      ...
      default:
        ...
        pHandle->SensorIsReliable = false;
				...
				pHandle->_Super.buffer_1[pHandle->_Super.buffer_pointer] =  pHandle->HallState;
				if (pHandle->_Super.buffer_pointer < 9)
			  pHandle->_Super.buffer_pointer ++;
        break;
    }
    ...
    else
    {
          ...
          if ( hHighSpeedCapture < LOW_RES_THRESHOLD ) /* If capture range correct */
          {
            if ( LL_TIM_GetPrescaler ( TIMx ) > 0u ) /* or prescaler cannot be further reduced */
            {
              ...
              pHandle->RatioDec = true;
				...
				pHandle->wCaptBuf_Delta = wCaptBuf - pHandle->wCaptBuf_Prev;
      ...
      /* the HALL_MAX_PSEUDO_SPEED is temporary in the buffer, and never included in average computation*/
        if ( wCaptBuf < pHandle->MinPeriod )
        {
					...
					pHandle->error_5 = true;
           /* pHandle->AvrElSpeedDpp = HALL_MAX_PSEUDO_SPEED; */
        }
				//else if( pHandle->wCaptBuf_Delta > pHandle->Max_CaptureBuffer_Delta )
				//{
				//	pHandle->error_4 = true;
				//}	
        else
        {
          ...
          if ( pHandle->SensorIsReliable) 
          {
            ...
            { /* Average speed allow to smooth the mechanical sensors misalignement */
              pHandle->AvrElSpeedDpp = ( int16_t )((int32_t) pHandle->PseudoFreqConv / ( pHandle->ElPeriodSum / pHandle->SpeedBufferSize )); /* Average value */
						...
						pHandle->previous_AvrElSpeedDpp = pHandle->AvrElSpeedDpp;
          }
          else /* Sensor is not reliable */
          {
            pHandle->AvrElSpeedDpp = 0;
				...
				pHandle->wCaptBuf_Prev = wCaptBuf;
...
__weak void * HALL_TIMx_UP_IRQHandler( void * pHandleVoid )
{
  ...
  if ( pHandle->SensorIsReliable )
  {
    ...
    hMaxTimerOverflow = ( uint16_t )( ( ( uint32_t )pHandle->HallTimeout * pHandle->OvfFreq )
		...
		if(hMaxTimerOverflow>UINT8_MAX)
		{hMaxTimerOverflow = UINT8_MAX;}
