...
void md_comm_init(MD_Comm_Handle_t * pMDcomm)
{
	...
	MD_Init(pMDcomm->pMD[M1]);
	MD_Init(pMDcomm->pMD[M2]);
				...
				case MC_PROTOCOL_CODE_GET_REG:
						...
						switch (tx_frame->Buffer[0])
						{
							case MC_PROTOCOL_REG_TORQUE_KP:
								MD_setTorqueKp(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_TORQUE_KI:
								MD_setTorqueKi(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_FLUX_KP:
								MD_setFluxKp(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_FLUX_KI:
								MD_setFluxKi(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_BUS_VOLTAGE:
								MD_setBusVoltage(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_TORQUE_MEAS:
								if (toSet > INT16_MAX)
									MD_setIqMeas(m_pMDcomm->pMD[bMotorSelection], UINT16_MAX-toSet);
								else
									MD_setIqMeas(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_FLUX_MEAS:
								if (toSet > INT16_MAX)
									MD_setIdMeas(m_pMDcomm->pMD[bMotorSelection], UINT16_MAX-toSet);
								else
									MD_setIdMeas(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_HEATS_TEMP:
								MD_setHeatsinkTemp(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_SPEED_MEAS:
								toSet += (rx_frame->Buffer[2] << 16) + (rx_frame->Buffer[3] << 24);
								MD_setSpeedMeas(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_FLAGS:
								MD_setFlags(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
							
							case MC_PROTOCOL_REG_STATUS:
								MD_setStatus(m_pMDcomm->pMD[bMotorSelection], toSet);
								break;
