...
__weak void MCboot( MCI_Handle_t* pMCIList[NBR_OF_MOTORS],MCT_Handle_t* pMCTList[NBR_OF_MOTORS] )
{
  ...
  STO_PLL_Init (&STO_PLL_M2);

  /******************************************************/
  /*   Speed & torque component initialization          */
  /******************************************************/
  STC_Init(pSTC[M2],pPIDSpeed[M2], &STO_PLL_M2._Super);
  /****************************************************/
  /*   Virtual speed sensor component initialization  */
  /****************************************************/
  ...
  MCT[M2].pSpeedSensorMain = (SpeednPosFdbk_Handle_t *) &STO_PLL_M2;
...
__weak void TSK_MediumFrequencyTaskM2(void)
{
  ...
  (void) STO_PLL_CalcAvrgMecSpeedUnit( &STO_PLL_M2, &wAux );
  ...
  case START:
    {
      ...
      if ( ObserverConverged )
      {
        qd_t StatorCurrent = MCM_Park( FOCVars[M2].Ialphabeta, SPD_GetElAngle( &STO_PLL_M2._Super ) );
  ...
  case START_RUN:
 /* only for sensor-less control */
    STC_SetSpeedSensor(pSTC[M2],&STO_PLL_M2._Super);  /*Observer has converged*/
    {
...
__weak uint8_t TSK_HighFrequencyTask(void)
{
  ...
  else
  {
    ...
    else // bMotorNbr != M1
    {
      ...
      STO_PLL_CalcElAngle (&STO_PLL_M2, &STO_Inputs);
      STO_PLL_CalcAvrgElSpeedDpp (&STO_PLL_M2); /*  Only in case of Sensor-less */
	  if (IsAccelerationStageReached == false)
      {
      ...
      if((hState == START) || (hState == SWITCH_OVER) || (hState == START_RUN)) /*  only for sensor-less*/
      {
        int16_t hObsAngle = SPD_GetElAngle(&STO_PLL_M2._Super);
