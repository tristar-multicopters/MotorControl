...
// This handler is called when there is a toggle event on encoder pin
	 ...
	 else
	 {
			...
			if(!m_pPAS_Handle->pas_state && m_pPAS_Handle->pas_level<=m_pPAS_Handle->hNumberOfLevel)
			{
				...
				m_pPAS_Handle->hTrefPAS = ((m_pPAS_Handle->hIqmax*m_pPAS_Handle->pas_level)/m_pPAS_Handle->hNumberOfLevel);
...
void PAS_Init(PAS_Handle_t* pHandle)
{
  ...
  uint32_t timer_task_addr;
	...
	m_pPAS_Handle->pas_state = false;
	m_pPAS_Handle->pas_level = PAS_LEVEL_3;
	m_pPAS_Handle->hTref = 0;
...
int16_t PAS_CalcIqref(PAS_Handle_t* pHandle, int16_t Tref)
{
  ...
  int16_t hAux;
	...
	if(Tref == 0 && pHandle->pas_state == true)
		hAux = pHandle->hTrefPAS;
	else
		hAux = Tref;

	wtemp =  ( int32_t )( pHandle->hLowPassFilterBW ) - 1u;
	wtemp *= ( int32_t ) ( pHandle->hTref );
	wtemp += hAux;
	wtemp /= ( int32_t )( pHandle->hLowPassFilterBW );

	pHandle->hTref = ( int16_t ) wtemp;
