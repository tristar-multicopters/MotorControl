/**
  ******************************************************************************
  * @file    main.c
  * @author  Sami Bouzid, FTEX
  * @brief   This file is the main application of the motor drive - Master
  *
	******************************************************************************
	*/

#include "nrf_delay.h"

#include "md_comm.h"
#include "host_comm.h"
#include "throttle.h"
#include "vc_tasks.h"

#include "RTE_Components.h"
#include  CMSIS_device_header
#include <cmsis_os2.h>
#include <rtx_os.h>
#include "EventRecorder.h"

extern osThreadId_t TSK_MDcomm_handle;
extern osThreadId_t TSK_FastLoopMD_handle;
extern osThreadId_t TSK_SlowLoopMD_handle;
//extern osThreadId_t TSK_ReplyToHost_handle;
//extern osThreadId_t TSK_usbd_handle;
extern osThreadId_t TSK_CANmsgTX_handle_t;
//extern TaskHandle_t TSK_MDreceiveFrames_handle;
//extern TaskHandle_t TSK_MDsendFrames_handle;

//****************** THREAD ATTRIBUTES ******************//

static const osThreadAttr_t ThAtt_FastLoop = {
	.name = "TSK_Fast",
	.stack_size = 512,
	.priority = osPriorityAboveNormal2
};

static const osThreadAttr_t ThAtt_SlowLoop = {
	.name = "TSK_Low",
	.stack_size = 512,
	.priority = osPriorityAboveNormal1
};

static const osThreadAttr_t ThAtt_MDComm = {
	.name = "TSK_MDCom",
	.stack_size = 512,
	.priority = osPriorityNormal2
};

static const osThreadAttr_t ThAtt_CANmsgTX = {
	.name = "TSK_CANBUS",
	.stack_size = 512,
	.priority = osPriorityNormal1
};

//static const osThreadAttr_t ThAtt_ReplyToHost = {
//	.name = "TSK_MDCom",
//	.stack_size = 512,
//	.priority = osPriorityNormal1
//};

//static const osThreadAttr_t ThAtt_USBD = {
//	.name = "TSK_MDCom",
//	.stack_size = 512,
//	.priority = osPriorityAboveNormal7
//};

/**************************************************************/

/**
 * @brief Function for main application entry.
 */
int main(void)
{	
	//nrf_delay_ms(100);
	SystemCoreClockUpdate();
	VC_BootUp();
	
	osKernelInitialize();  // Initialise the kernel
	EventRecorderInitialize(EventRecordAll,1U); // Initialise the events
	/* Create task to manage communication between nRF and STM */
	TSK_MDcomm_handle      = osThreadNew(TSK_MDcomm, 
																			NULL,
																			&ThAtt_MDComm);
	/* Create task to manage vehicle state and throttle input */
	TSK_FastLoopMD_handle  = osThreadNew(TSK_FastLoopMD,
																			NULL, 
																			&ThAtt_FastLoop);
//	/* Create task to get regs values with low priority from STM */
	TSK_SlowLoopMD_handle  = osThreadNew(TSK_SlowLoopMD,
																			 NULL,
																			 &ThAtt_SlowLoop);
//	/* Create task communicate with host */
//	TSK_ReplyToHost_handle = osThreadNew(TSK_ReplyToHost,
//																			 NULL,
//																			 &ThAtt_ReplyToHost);
//	/* Create task to communicate with host via USBD module*/													 
//	TSK_usbd_handle 			 = osThreadNew(TSK_init_usbd,
//																			 NULL,
//																			 &ThAtt_USBD);
//	/* Create task to manage CAN Protocol */																		 
	TSK_CANmsgTX_handle_t  = osThreadNew(TSK_CANmsg, 
																			 NULL, 
																			 &ThAtt_CANmsgTX);
	
	// Start thread execution
	if (osKernelGetState() == osKernelReady)
	{
    osKernelStart();                    								
  }
	
	while (true)
	{
		// We should never get here...
	}
}


/** @} */
