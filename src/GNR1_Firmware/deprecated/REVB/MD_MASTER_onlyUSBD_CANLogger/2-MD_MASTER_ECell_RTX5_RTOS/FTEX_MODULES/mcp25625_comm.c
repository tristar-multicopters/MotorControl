...
static const osMessageQueueAttr_t queueAtr_can_msg = {
		.name = "Can_msg_Q",
};
/****************************************************/
osThreadId_t TSK_CANmsgTX_handle_t;
...
static void MCP25625_IT_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
	...
	osThreadFlagsSet(TSK_CANmsgTX_handle_t, CANBUS_FLAG);
}
...
/** @brief  Task function for managing the CAN instance 
		@p  		pvParameter  non-used parameter
*/
__NO_RETURN void TSK_CANmsg(void * pvParameter)
{
	UNUSED_PARAMETER(pvParameter);
	CANTX_queue = osMessageQueueNew(MCP_CANMSG_SIZE, sizeof(CAN_toSend_t), &queueAtr_can_msg);
	
	while(true)
	{
		osThreadFlagsWait(CANBUS_FLAG, osFlagsWaitAny, 100);
		
		CAN_toSend_t send_can_message;
		
		// If an interrupt from MCP26526 has been received
		if(m_pMCPHandle->IT_received)
		{
			MCP25625_manage_IT();
			if(m_pMCPHandle->state == MCP_ERROR_MERRF ||
				 m_pMCPHandle->state == MCP_ERROR_ERRIF)
			{
				MCP25625_check_errors();
			}
		}
		
		// Send message to the CAN BUS if there's no ongoing transfer
		if(!m_pMCPHandle->ongoing_transfer)
		{
			if(osMessageQueueGet(CANTX_queue, & send_can_message, NULL, 0) == osOK)
			{
		...
		// Process message received from the CAN bus
		if(!nrf_queue_is_empty(m_pMCPHandle->MCP_queue))
		{
			CAN_Message_t message;
			if(NRF_SUCCESS == MCP25625_popCANmsg(&message))
				MCP25625_ProcessRXMsg(message);
		}
	}
}

/** @brief Function for managing the signal interrupt comming from the MCP.
		The interrupt could be generated when :
		- a CAN message has been loaded to one of the TX buffers of the MCP
    - a CAN message has been received to one of the RX buffers of the MCP
	  - there was an error in the CAN bus. 
*/
void MCP25625_manage_IT( void )
{
	...
	if(m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_RX0IF) // Receive message on the RX0 buffer
	{
		...
		if(m_pMCPHandle->state == MCP_ERROR_ERRIF || m_pMCPHandle->state == MCP_ERROR_MERRF)
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_RX0IF | MCP_ERRIF | MCP_MERRF, 0x0, true);
		else
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF,MCP_RX0IF, 0x0, true);
		
		MCP25625_ReadRxBuffer(m_pMCPHandle,MCP_READ_RX0);
		m_pMCPHandle->state = MCP_RX0_RECEIVED;
	...
	{
		// Clear the RX1IF flag to be able to receive futur messages
		if(m_pMCPHandle->state == MCP_ERROR_ERRIF || m_pMCPHandle->state == MCP_ERROR_MERRF)
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_RX1IF | MCP_ERRIF | MCP_MERRF, 0x0, true);
		else
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF,MCP_RX1IF, 0x0, true);
		
		MCP25625_ReadRxBuffer(m_pMCPHandle,MCP_READ_RX1);
		m_pMCPHandle->state = MCP_RX1_RECEIVED;
	}
	
	if(m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_TX0IF) // Transmit a message
	{
		if(m_pMCPHandle->state == MCP_ERROR_ERRIF || m_pMCPHandle->state == MCP_ERROR_MERRF)
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_TX0IF | MCP_ERRIF | MCP_MERRF, 0x0, true);
		else
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_TX0IF, 0x0, true);
		
		m_pMCPHandle->state = MCP_TX0_RECEIVED;
		m_pMCPHandle->ongoing_transfer = false;
	}
	// Rise error flags
	else
	{
		if (m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_ERRIF)
		{
			m_pMCPHandle->state = MCP_ERROR_ERRIF;
		}
		else if (m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_MERRF)
		{
			m_pMCPHandle->state = MCP_ERROR_MERRF;
		}
	}
}	

/** @brief Function for checking if there are errors comming from the MCP.
		It clears he CANINTF register if needed and can put lower the ongoing transfert flag of the MCP
*/
void MCP25625_check_errors( void )
{
	MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF,
																 MCP_ERRIF | MCP_MERRF, 0x00, true);

	MCP25625_ReadRegister(m_pMCPHandle, MCP_CANINTF, true);
	if(!m_pMCPHandle->reg_table[MCP_CANINTF] & (MCP_ERRIF | MCP_MERRF))
		m_pMCPHandle->ongoing_transfer = false;
	// Sometimes TX0 flags rises without producing an interruption...
	else if(m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_TX0IF)
	{
		MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF,
																MCP_TX0_INT, 0x00, true);
		m_pMCPHandle->ongoing_transfer = false;
	}
}

/**@brief Function for initialising the MCP instance
	 @p p_Handle: Handle of the MCP
	 @return returns CAN_OK if instance was well initialised and CAN_FAILINIT
		       if it couldn't be initialised
*/
...
/**@brief Function to write a CAN message before sending to the MCP25625
	 @p p_Handle: Handle of the MCP
   @return returns message to send with the size of elements to send.
*/
...
void MCP25625_WriteCANmsg(MCP25625_Handle_t* p_Handle, CAN_TX_Buffer_t buffer, CAN_Message_t* pMsg)
{
	...
	if(osMessageQueuePut(CANTX_queue, &toSend, NULL, 0) == osOK)
	{
		osThreadFlagsSet(TSK_CANmsgTX_handle_t, CANBUS_FLAG);
	}
	else
	{
		p_Handle->hError = MCP_QUEUE_FULL;
	}
}

/**@brief Function for pull a CAN message from the queue
	 @p message: Pointer to the message destination
	 @return returns NRF_SUCCESS if an element was returned or NRF_ERROR_NOT_FOUND if
	 there's no elements in the queue.
*/

ret_code_t MCP25625_popCANmsg( CAN_Message_t * message )
{
	return nrf_queue_generic_pop(&m_MCP_queue,message,false);
}

/** @brief  Function for sending the motor drive temperature
		@p 			message      received CAN message
*/
void MCP25625_ProcessRXMsg(CAN_Message_t message)
{
	switch(message.id)
	{
		case CAN_LOCK:
			if(message.data[0] == 1)
				nrf_gpio_pin_set(m_pMCPHandle->lock_pin);
			else if(message.data[0] == 2)
				nrf_gpio_pin_clear(m_pMCPHandle->lock_pin);
			break;
		default:
			break;
	}
