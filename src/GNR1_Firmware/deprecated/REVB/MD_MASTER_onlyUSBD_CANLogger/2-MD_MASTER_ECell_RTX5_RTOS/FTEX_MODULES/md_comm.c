				...
				case MC_PROTOCOL_CODE_GET_REG:
						...
						switch (tx_frame->Buffer[0])
						{
							case MC_PROTOCOL_REG_TORQUE_KP:
								MD_setTorqueKp(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_TORQUE_KI:
								MD_setTorqueKi(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_FLUX_KP:
								MD_setFluxKp(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_FLUX_KI:
								MD_setFluxKi(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_SPEED_KP:
								MD_setSpeedKp(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_SPEED_KI:
								MD_setSpeedKi(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_BUS_VOLTAGE:
								MD_setBusVoltage(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_TORQUE_MEAS:
								MD_setIqMeas(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_FLUX_MEAS:
								MD_setIdMeas(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_HEATS_TEMP:
								MD_setMotorTemp(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_SPEED_MEAS:
								toSet += (rx_frame->Buffer[2] << 16) + (rx_frame->Buffer[3] << 24);
								MD_setSpeedMeas(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_SPEED_REF:
								toSet += (rx_frame->Buffer[2] << 16) + (rx_frame->Buffer[3] << 24);
								MD_setSpeedRef(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_FLAGS:
								MD_setFlags(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_CONTROL_MODE:
								MD_setMode(m_pMDcomm, bMotorSelection, toSet);
								break;
							
							case MC_PROTOCOL_REG_STATUS:
								MD_setStatus(m_pMDcomm, bMotorSelection, toSet);
								break;
...
__NO_RETURN void TSK_MDcomm (void * pvParameter)
{
	...
	pending_frame_queue = osMessageQueueNew(MD_BUFFER_SIZE, sizeof(FCP_Frame_t),&queueAtr_pending_frame); 
	transaction_done_queue = osMessageQueueNew(MD_BUFFER_SIZE, sizeof(frame_transaction_t),&queueAtr_transaction_done);
	
	timeout_timer = osTimerNew(&md_timeout_callback, osTimerPeriodic, NULL, &TmrAtt_timeout_timer);										
