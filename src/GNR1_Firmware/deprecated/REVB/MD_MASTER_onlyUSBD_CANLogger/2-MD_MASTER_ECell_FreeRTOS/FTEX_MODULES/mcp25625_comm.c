...
void MCP25625_manage_IT( void )
{
	...
	if(m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_RX0IF) // Receive message on the RX0 buffer
	{
		...
		if(m_pMCPHandle->state == MCP_ERROR_ERRIF || m_pMCPHandle->state == MCP_ERROR_MERRF)
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_RX0IF | MCP_ERRIF | MCP_MERRF, 0x0, false);
		else
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF,MCP_RX0IF, 0x0, false);
	...
	else if(m_pMCPHandle->reg_table[MCP_CANINTF] & MCP_RX1IF) // Receive message on the RX1 buffer
	{
		...
		if(m_pMCPHandle->state == MCP_ERROR_ERRIF || m_pMCPHandle->state == MCP_ERROR_MERRF)
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_RX1IF | MCP_ERRIF | MCP_MERRF, 0x0, false);
		else
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF,MCP_RX1IF, 0x0, false);
	...
	{
		if(m_pMCPHandle->state == MCP_ERROR_ERRIF || m_pMCPHandle->state == MCP_ERROR_MERRF)
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_TX0IF | MCP_ERRIF | MCP_MERRF, 0x0, false);
		else
			MCP25625_bitModifyRegister(m_pMCPHandle, MCP_CANINTF, MCP_TX0IF, 0x0, false);
