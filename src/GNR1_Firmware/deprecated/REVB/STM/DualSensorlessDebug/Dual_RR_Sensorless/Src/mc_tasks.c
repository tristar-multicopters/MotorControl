...
__weak void MCboot( MCI_Handle_t* pMCIList[NBR_OF_MOTORS],MCT_Handle_t* pMCTList[NBR_OF_MOTORS] )
{
  ...
  /********************************************************/
  pBusSensorM1 = &VirtualBusVoltageSensorParamsM2; /* powerboard configuration: Rdivider or Virtual*/
  ...
  STO_CR_Init (&STO_CR_M2);

  /******************************************************/
  /*   Speed & torque component initialization          */
  /******************************************************/
  STC_Init(pSTC[M2],pPIDSpeed[M2], &STO_CR_M2._Super);
  /****************************************************/
  /*   Virtual speed sensor component initialization  */
  /****************************************************/
  ...
  MCT[M2].pSpeedSensorMain = (SpeednPosFdbk_Handle_t *) &STO_CR_M2;
...
__weak void TSK_MediumFrequencyTaskM2(void)
{
  ...
  (void) STO_CR_CalcAvrgMecSpeedUnit( &STO_CR_M2, &wAux );
  ...
  case START:
	...
	ObserverConverged = STO_CR_IsObserverConverged( &STO_CR_M2,hForcedMecSpeedUnit );
      ...
      if ( ObserverConverged )
      {
        qd_t StatorCurrent = MCM_Park( FOCVars[M2].Ialphabeta, SPD_GetElAngle( &STO_CR_M2._Super ) );
  ...
  case START_RUN:
 /* only for sensor-less control */
    STC_SetSpeedSensor(pSTC[M2],&STO_CR_M2._Super);  /*Observer has converged*/
    {
...
__weak uint8_t TSK_HighFrequencyTask(void)
{
  ...
  else
  {
    ...
    else // bMotorNbr != M1
    {
      ...
      STO_CR_CalcElAngle (&STO_CR_M2, &STO_Inputs);
      STO_CR_CalcAvrgElSpeedDpp (&STO_CR_M2); /*  Only in case of Sensor-less */
      hState = STM_GetState(&STM[M2]);
      if((hState == START) || (hState == SWITCH_OVER) || (hState == START_RUN)) /*  only for sensor-less*/
      {
        int16_t hObsAngle = SPD_GetElAngle(&STO_CR_M2._Super);
...
LL_GPIO_LockPin(M2_CURR_ICS_U_GPIO_Port, M2_CURR_ICS_U_Pin);
LL_GPIO_LockPin(M2_CURR_ICS_V_GPIO_Port, M2_CURR_ICS_V_Pin);
LL_GPIO_LockPin(M1_CURR_ICS_V_GPIO_Port, M1_CURR_ICS_V_Pin);
LL_GPIO_LockPin(M1_CURR_ICS_U_GPIO_Port, M1_CURR_ICS_U_Pin);
