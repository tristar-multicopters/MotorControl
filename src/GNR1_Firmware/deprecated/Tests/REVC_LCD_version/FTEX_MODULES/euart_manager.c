...
static ret_code_t m_code = NRF_SUCCESS;
...
{	// Verify if instance was already initialised
	...
	{
		nrf_drv_uart_uninit (pHandle->p_uart_inst);
		nrf_drv_uart_init(pHandle->p_uart_inst, &uart_config, euart_event_handler);
	}
	else
	{
...
{
	m_code = nrf_drv_uart_rx(pHandle->p_uart_inst, rx_buffer, 1);
	if(m_code != NRFX_SUCCESS)
	{
		return;
	}
