...
#define MCP_QUEUE_FULL			(uint16_t)0x00001

#define MCP_CANMSG_SIZE   64
#define CANBUS_FLAG	0x02
...
/*--------------------------------- FUNCTIONS ---------------------------------*/
/* Function for initialise the MCP25625 instance */
uint8_t MCP25625_Init(MCP25625_Handle_t* p_Handle);

/* Function to prepare a CAN message before sending it */
void MCP25625_WriteCANmsg(MCP25625_Handle_t* p_Handle, CAN_TX_Buffer_t buffer, CAN_Message_t* pMsg);

/* Function for loading the CAN message into the TX buffer of the MCP25625 */
void MCP25625_LoadTxBuffer(MCP25625_Handle_t* p_Handle, uint8_t length, uint8_t *buffer_tx);

/* Function for reading a CAN message from the RX buffer of the MCP25625*/
void MCP25625_ReadRxBuffer(MCP25625_Handle_t* p_Handle, uint8_t buffer_ID);

/* Function to request sending a CAN message to the CAN bus */
void MCP25625_SendCANmsg(MCP25625_Handle_t* p_Handle, CAN_TX_Buffer_t buffer);

/* Function for managing the signal interrupt comming fromthe MCP*/
void MCP25625_manage_IT( void );

/* Function for checking if there are errors comming from the MCP*/
void MCP25625_check_errors( void );

/* Function for popping a CAN message from the queue that keeps all the received messages from the CAN bus*/
ret_code_t MCP25625_popCANmsg( CAN_Message_t * message );

/* Function for processing the received CAN messages */
void MCP25625_ProcessRXMsg(CAN_Message_t message);

///* Function for updating the ongoing transfert flag of the MCP instance*/
//void MCP25625_setOnGoingTrasfert( bool isOnGoing);
