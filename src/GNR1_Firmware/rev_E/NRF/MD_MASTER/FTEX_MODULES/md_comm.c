				...
				case MC_PROTOCOL_CODE_GET_REG:
						...
						{							
							...
							case MC_PROTOCOL_REG_FLAGS:
								m_pMDcomm->pMD[bMotorSelection]->MDStateMachine.hMFaultNow = toSet & 0xFF;
								m_pMDcomm->pMD[bMotorSelection]->MDStateMachine.hMFaultOccurred = (toSet >> 8) & 0xFF;
								break;
...
uint8_t md_faultAcknowledge(uint8_t motorSelection)
{
	FCP_Frame_t frame;
	frame.Code = ( (motorSelection+1) << 5 ) + MC_PROTOCOL_CODE_EXECUTE_CMD;
	frame.Size = 1;
