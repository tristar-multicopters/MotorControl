...
#define TORQUE_RAMP_DURATION									250
#define STARTUP_ENABLE 												1
#define RESTART_MAX_ATTEMPT										5
...
{	
  ...
  ////////////////FOR TESTING//////////////////////////
	VController.euart_type = EUART_APT;
	////////////////////////////////////////////////////
}
	...
	while (true)
	{
		...
		switch ( StateVC )
		{
			...
			case V_STANDBY_START:
					...
					bRestartAttemptedM1 = 0, bRestartAttemptedM2 = 0;
					if(osTimerIsRunning(STMTimeoutTimer))
						 osTimerStop(STMTimeoutTimer);
					wReturn2StandbyCounterM1 = 0, wReturn2StandbyCounterM2 = 0, bReturn2Standby = false;
					#if USE_MOTOR1
					md_setTorqueRamp(M1, hTref1, TORQUE_RAMP_DURATION);
					md_startMotor(M1);
					#endif
					#if USE_MOTOR2
					md_setTorqueRamp(M2, hTref2, TORQUE_RAMP_DURATION);
					md_startMotor(M2);
					#endif
					#if STARTUP_ENABLE
			...
			case V_START_FORWARD:
					...
					switch ( pMDrive1->MDStateMachine.bMState )
					{
						case M_RUN:
							...
							break;
						case M_IDLE:
							if ( bRestartAttemptedM1 < RESTART_MAX_ATTEMPT )
							{
								md_setTorqueRamp(M1, hTref1, TORQUE_RAMP_DURATION);
								md_startMotor(M1);
								bRestartAttemptedM1++;
					...
					switch ( pMDrive2->MDStateMachine.bMState )
					{
						case M_RUN:
							...
							break;
						case M_IDLE:
							if ( bRestartAttemptedM2 < RESTART_MAX_ATTEMPT )
							{
								md_setTorqueRamp(M2, hTref2, TORQUE_RAMP_DURATION);
								md_startMotor(M2);
								bRestartAttemptedM2++;
			...
			case V_RUN_FORWARD:
					...
					switch ( pMDrive2->MDStateMachine.bMState )
					{
						...
						if (wReturn2StandbyCounterM1 > RETURN_TO_STANDBY_LOOPTICKS && !bReturn2Standby)
						{
							bReturn2Standby = true;
							wReturn2StandbyCounterM1 = 0;
							#if USE_MOTOR1
							md_stopMotor(M1);
							#endif
						...
						if (wReturn2StandbyCounterM2 > RETURN_TO_STANDBY_LOOPTICKS && !bReturn2Standby)
						{
							bReturn2Standby = true;
							wReturn2StandbyCounterM2 = 0;
							#if USE_MOTOR1
...
__NO_RETURN void TSK_eUART (void * pvParameter)
{
	...
	switch(dev_type)
	{
		...
		case EUART_FTEX:
			LCD_FTEX_init(&VController);
			break;
		
		case EUART_APT:
			LCD_APT_init(&VController);
			break;
		default:
			break;
	}
