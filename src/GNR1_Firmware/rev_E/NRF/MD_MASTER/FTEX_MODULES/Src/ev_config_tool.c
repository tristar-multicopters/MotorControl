...
static EVCT_handle_t m_EV_handler = {0};	// EVCT handler
...
static void EV_init_handler(EVCT_handle_t * pHandle);
/*********************************************************************/
...
void EVCT_init(VCI_Handle_t* pHandle)
{
	...
	eUART_Init(&m_EV_handler.euart_handler,uart_EVCT_config);
	eUART_Receive(&m_EV_handler.euart_handler, m_EV_handler.euart_handler.rx_byte);	
}
