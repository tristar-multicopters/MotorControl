...
{	
	pHandle->hEstElAngle = pHandle->pHallSensor->_Super.hElAngle;
	pHandle->hEstElSpeedDpp = pHandle->pHallSensor->_Super.hElSpeedDpp;
	pHandle->hEstMechTorque = 0;

	pHandle->wEstElAngle = pHandle->pHallSensor->_Super.hElAngle*INT16_MAX;
	pHandle->wEstElSpeedDpp = pHandle->pHallSensor->_Super.hElSpeedDpp*INT16_MAX;
...
int16_t RotorPosObs_CalcElAngle(RotorPositionObserverHandle_t * pHandle, int16_t hElTorque)
{
	...
	hErrorSin = MCM_Trig_Functions(pHandle->pHallSensor->_Super.hElAngle - pHandle->hEstElAngle).hSin;
...
{
	bool bIsReliable = SPD_Check(&pHandle->pHallSensor->_Super);
	bIsReliable &= SPD_IsMecSpeedReliable (&pHandle->Super, pMecSpeedUnit);
