...
CO_ERR COSdoDownloadSegmented(CO_SDO *srv)
{
    ...
    if ((cmd & 0x01) == 0x01) {
        ...
        if (srv->Obj->Type != 0) {
            ...
            result = COObjWrValue(srv->Obj, srv->Node, srv->Buf.Start, (uint8_t)len, srv->Node->NodeId);
        }
        if (result != CO_ERR_NONE) {
            srv->Node->Error = CO_ERR_SDO_WRITE;
            COSdoAbort(srv, CO_SDO_ERR_HW_ACCESS);
    ...
    if (num > 0u) {
        if (srv->Blk.Size > num) {
            ...
            if (srv->Obj->Type != 0) {
                ...
                err = COObjRdValue(srv->Obj, srv->Node, srv->Buf.Cur, (uint8_t) srv->Blk.Size, srv->Node->NodeId);
            }
            if (err != CO_ERR_NONE) {
                srv->Node->Error = CO_ERR_SDO_READ;
            }
