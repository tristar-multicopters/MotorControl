...
void uCAL_UART_SetBaudRate(UART_Handle_t *pHandle)
{
    ...
    err = R_SCI_B_UART_BaudSet(pHandle->UARTInstance->p_ctrl, (void *) &baud_setting);
    ASSERT(FSP_SUCCESS == err);
...
void uCAL_UART_Transmit(UART_Handle_t *pHandle, uint8_t *Buffer, uint32_t BufferSize)
{
    ...
    fsp_err_t err = R_SCI_B_UART_Write(pHandle->UARTInstance->p_ctrl, Buffer, BufferSize);
    ASSERT(FSP_SUCCESS == err);    
}
...
void uCAL_UART_Receive(UART_Handle_t *pHandle, uint8_t *Buffer, uint32_t BufferSize)
{
    ...
    fsp_err_t err = R_SCI_B_UART_Read(pHandle->UARTInstance->p_ctrl, Buffer, BufferSize);
    ASSERT(FSP_SUCCESS == err);
}
