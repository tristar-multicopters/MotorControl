...
uint8_t ExternalFash_Init(void)
{
    ...
    if(MX25_ResetEnable() != MX25L3233F_OK) 
        return FLASH_ERROR;

    R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MILLISECONDS);

    /* External Flash Memory software reset */
    if(MX25_ResetMemory() != MX25L3233F_OK) 
        return FLASH_ERROR;

    R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MILLISECONDS);
	
    /* External Flash Read ID */
	ExternalFash_ReadID(flash_id);
...
*/
uint8_t ExternalFash_EraseSector(uint32_t SectorAddr)
{
    /* Sector Erase */
    MX25_WriteEnable();

    MX25_BlockErase(SectorAddr, MX25L3233F_ERASE_4K);

    /* Wait the end of Flash writing and Deselect the FLASH */
    if(MX25_AutoPollingMemReady(MX25L3233F_GENERAL_TIME_OUT) != MX25L3233F_OK)
    {
        return FLASH_ERROR;
    }
    else
...
/**
  ...
  */
uint8_t ExternalFash_EraseChip(void)
{
    /* Sector Erase */
    MX25_WriteEnable();
    ...
    /* Wait the end of Flash writing and Deselect the FLASH */
    if(MX25_AutoPollingMemReady(MX25L3233F_CHIP_ERASE_MAX_TIME) != MX25L3233F_OK)
    {
        return FLASH_ERROR;
    }
    else
...
*/
uint8_t ExternalFash_WriteData(uint32_t WriteAddr, uint8_t* pData, uint32_t Size)
{
    ...
    /* Enable write operations */
        if(MX25L_WriteEnable() != MX25L3233F_OK)
        {
        ret = FLASH_ERROR;
        }/* Issue page program command */
        else if(MX25_PageProgram(pData, current_addr, current_size) != MX25L3233F_OK)
        {
            ret = FLASH_ERROR;
        }/* Configure automatic polling mode to wait for end of program */
        else if(MX25_AutoPollingMemReady(MX25L3233F_GENERAL_TIME_OUT) != MX25L3233F_OK)
        {
            ret = FLASH_ERROR;
        }
...
{
    if(MX25_Read(pData, uwStartAddress, uwDataSize)!= MX25L3233F_OK)
    {
        return FLASH_ERROR;
    }
    else
...
{
    if(MX25_ReadID(pData) != MX25L3233F_OK)
    {
        return FLASH_ERROR;
    }
    else
